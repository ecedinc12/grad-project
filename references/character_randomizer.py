...
class GoTo(GoToCommand):
    ...
    def randomize(
        ...
        return super().randomize(
            agent, agent_speed, agent_pos_dict, navmesh, interactable_objects, rng, navigation_area, config
        )
...
class CharacterRandomizer(Randomizer):
    ...
    def load_command_transition_map(self, file_path):
        ...
        # Guard against missing or invalid transition map file
        if not isinstance(json_data, dict) or not json_data:
            carb.log_warn(
                f"Character command transition map is missing or invalid: '{file_path}'. Using default map."
            )
            # Build a default transition map from built-in commands with uniform weights
            default_command_names = [cmd.name for cmd in CHARACTER_BUILD_IN_COMMANDS]
            default_transitions = {name: 1.0 for name in default_command_names}
            json_data = {
                name: {"weight": 1.0, "transitions": dict(default_transitions)} for name in default_command_names
            }
        try:
            self.transition_map = CommandTransitionMap(json_data)
        except Exception as ex:
            carb.log_warn(
                f"Failed to parse character command transition map '{file_path}': {ex}. Using default map."
            )
            default_command_names = [cmd.name for cmd in CHARACTER_BUILD_IN_COMMANDS]
            default_transitions = {name: 1.0 for name in default_command_names}
            default_json = {
            ...
            self.transition_map = CommandTransitionMap(default_json)
