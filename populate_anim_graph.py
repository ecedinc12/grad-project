...
def populate_timing_template(
    ...
    anim_trans_to_condition_prim = populate_action_condition(stage, anim_trans_to_prim, custom_command.name)
    ...
    anim_trans_back_condition_prim = populate_action_condition(stage, anim_trans_back_prim, "None")
...
def populate_goto_blend_template(
    ...
    biped_prim_path = biped_prim.GetPrimPath()
    # Check if anim garph is valid
    anim_graph_prim = get_anim_graph_prim(biped_prim)
    if not anim_graph_prim.IsValid():
        carb.log_error(
    ...
    anim_clip_prim = populate_animation_clip(stage, anim_clip_prim_path, anim_prim, custom_command)
    #   4.2 Filter node
    ...
    # omni.kit.commands.execute("CreateUsdAttribute", prim=anim_graph_prim, attr_name="anim:graph:variable:NewVariable", attr_type=Sdf.ValueTypeNames.Float, attr_value=0.0)
    # omni.kit.commands.execute("RenameAnimationGraphVariableAttributeCommand", prim=anim_graph_prim, old_attr_name="anim:graph:variable:NewVariable", new_attr_name="anim:graph:variable:Value0")
    omni.kit.commands.execute(
    ...
    omni.kit.commands.execute(
        "AddRelationshipTargetCommand", relationship=blend_prim.GetRelationship("inputs:pose0"), target=filter_prim_path
    )
    omni.kit.commands.execute(
        "AddRelationshipTargetCommand",
